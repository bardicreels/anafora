...
# Sort the vtt_data dictionary by the "name" field
sorted_vtt_data = OrderedDict(sorted(vtt_data.items(), key=lambda x: x[1]['name'].lower()))

# Write the sorted data to a JSON file
output_path = os.path.join(project_root, 'data', 'vtt_content.json')
with open(output_path, 'w', encoding='utf-8') as file:
    json.dump(sorted_vtt_data, file, indent=2, ensure_ascii=False)

print(f"Created {output_path} with VTT contents, sorted alphabetically by name")
